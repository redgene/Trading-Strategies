...
class FamaFrenchFiveFactorsAlgorithm(QCAlgorithm):
    ...
    def OnData(self, data):
        ...
        # new added line: wait for our macd and RSI to fully initialize
    ...
    def PricePercentageChange(self):
        pass
    
    def PurchaseVolumeChange(self, coarse):
        self.AddData(NVDA, "PutCallRatio", Resolution.Daily)
        
        # Complete Add Option API - Including Default Parameters:
        # self.option = self.AddOption(string underlying, Resolution resolution = Resolution.Minute, string market = null, bool fillDataForward = true, decimal leverage = 0m)
        # load current option data for a single stock
    ...
    def MovingAvConvDiv(self):
        # define our daily macd(12,26) with a 9 day signal
        self.macd = self.MACD("SPY", 12, 26, 9, MovingAverageType.Exponential)
        
    def StocasticOscillator(self):
        self.sto = self.STO("SPY", 14, Resolution.Minute)
    
# Price increase or decrease as a percentage over given time period, e.g. 3 days


# Increase in stock/call/put option purchase volume over a given time period
# Also the distribution of strike prices in call or put option (put/call volume ratio > 1)
